...
for(var i in handlers)document[i]=handlers[i];var reLink=/https:\/\/sit.aces01.nus.edu.sg\/cors\/jsp\/report\/ModuleDetailedInfo.jsp\?acad_y=(.+)&sem_c=(\d)&mod_c=(.+)/,tt=new TimeTable,ripper=new Ripper,st=new Set,arrInterval=[],backgroundColor=["#fc9","#cc9","#c9f","#f9f","#cfc","#9c9","#99f","#999","#696","#606","#933","#009"],fontColor=["#000","#000","#000","#000","#000","#000","#000","#000","#000","#fff","#fff","#fff"];
...
function changeOpac(b,c){if(object=document.getElementById(c)){object=object.style;object.opacity=b/100;object.MozOpacity=b/100;object.KhtmlOpacity=b/100;object.filter="alpha(opacity="+b+")"}}function test(b){document.getElementById("tester").innerHTML=b}function testp(b){document.getElementById("tester").innerHTML+=b}function Module(){this.exam=this.link=this.code="";this.lecture=[];this.tutorial=[];this.laboratory=[]}Module.prototype.hasLecture=function(){return this.lecture.length>0};
Module.prototype.hasTutorial=function(){return this.tutorial.length>0};Module.prototype.hasLaboratory=function(){return this.laboratory.length>0};function Session(b,c,e,o,f,q){this.day=parseInt(b);this.start=parseInt(c);this.end=parseInt(e);this.type=parseInt(o);this.place=f;this.cell=q}function Part(b,c,e){this.title=b;this.type=c;this.session=e}
...
12)this.rip();else{b("#ripButton").val("Re-Scan All").mouseup(this.start);tt.module.length>0&&b("#nextButton").show();if(this.auto_start){tt.createTable();tt.createAllNode();st.showSetFunctions();showPage3();setTimeout("alert('Here you are. Happy testing! :)')",900)}}};q.MAX_RIP_INDEX=12;return q}($);function Set(){this.maxSlot=3;this.ripModule=[];this.ripMax=this.ripIndex=0;this.fixedArray=[];this.onTableArray=[]}
...
function setCookie(b,c,e,o,f,q){b=b+"="+encodeURIComponent(c);if(e)b+="; expires="+e.toGMTString();if(o)b+="; path="+o;if(f)b+="; domain="+f;if(q)b+="; secure";document.cookie=b}function getCookie(b){return RegExp("(?:; )?"+b+"=([^;]*);?").test(document.cookie)?decodeURIComponent(RegExp.$1):null}var NUSchedule=NUSchedule||{};
NUSchedule.signals=NUSchedule.signals||function(b){return{register:function(c,e){var o=function(){e.apply(o,Array.prototype.slice.call(arguments,1))};b(document).bind(c,o);return o},remove:function(c,e){b(document).unbind(c,e)},remove_all:function(c){b(document).unbind(c)},send:function(c,e){b(document).trigger(c,e)}}}(jQuery);var cellWidth=55,cellHeight=40;
...
TimeTable.prototype.getTab=function(b,c){theWeek=parseInt(b.substring(1,2));theTime=parseInt(b.substring(3,b.length));col=Math.floor(theTime/100)-8;row=theWeek-1;return this.cell[row][col].indexOf(c)};if(!Array.indexOf)Array.prototype.indexOf=function(b){for(var c=0;c<this.length;c++)if(this[c]==b)return c;return-1};String.prototype.trim=function(){for(var b=this.replace(/^\s\s*/,""),c=/\s/,e=b.length;c.test(b.charAt(--e)););return b.slice(0,e+1)};
function createP(){return document.createElement("p")}function createDiv(){return document.createElement("div")}
...
delete f.complete}f.success=function(k){return function(d){k&&k.call(this,d.results[0].replace(/<script[^>]+?\/>|<script(.|\s)*?\/script>/gi,""),"success",{responseText:d.results[0]})}}(f.success)}return b.apply(this,arguments)}}(jQuery.ajax);
