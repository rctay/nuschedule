jQuery.ajax=function(b){var c=location.protocol,f=RegExp(c+"//"+location.hostname),o="http"+(/^https/.test(c)?"s":"")+"://query.yahooapis.com/v1/public/yql?callback=?";return function(k){var q=k.url;if(/get/i.test(k.type)&&!/json/i.test(k.dataType)&&!f.test(q)&&/:\/\//.test(q)){k.url=o;k.dataType="json";k.data={q:'select * from html where url="{URL}" and xpath="*"'.replace("{URL}",q+(k.data?(/\?/.test(q)?"&":"?")+jQuery.param(k.data):"")),format:"xml"};if(!k.success&&k.complete){k.success=k.complete;
delete k.complete}k.success=function(e){return function(d){if(e){var g=d.results[0];if(g!=null)g=g.replace(/<script[^>]+?\/>|<script(.|\s)*?\/script>/gi,"");e.call(this,g,"success",{responseText:d.results[0]})}}}(k.success)}return b.apply(this,arguments)}}(jQuery.ajax);if(!Array.indexOf)Array.prototype.indexOf=function(b){for(var c=0;c<this.length;c++)if(this[c]==b)return c;return-1};
...
Module.prototype.hasTutorial=function(){return this.tutorial.length>0};Module.prototype.hasLaboratory=function(){return this.laboratory.length>0};function Session(b,c,f,o,k,q){this.day=parseInt(b);this.start=parseInt(c);this.end=parseInt(f);this.type=parseInt(o);this.place=k;this.cell=q}function Part(b,c,f){this.title=b;this.type=c;this.session=f}
var Ripper=function(b){var c=/(\w+)\s+From\s+(\d+)\s+hrs\s+to\s+(\d+)\s+hrs\s+in\s+(.+),/,f=function(){var e={MONDAY:1,TUESDAY:2,WEDNESDAY:3,THURSDAY:4,FRIDAY:5,SATURDAY:6,SUNDAY:7};return function(d){return e[d]}}(),o=function(){return function(e,d,g){e={acad_y:e,sem_c:d,mod_c:g};d=[];for(var s in e)d.push(s+"="+e[s]);return"https://aces01.nus.edu.sg/cors/jsp/report/ModuleDetailedInfo.jsp?"+d.join("&")}}(),k=function(){(function(e){NUSchedule.signals.register("on_module_rip_success",function(d){e.finish_job(d,
true)});NUSchedule.signals.register("on_module_rip_error",function(d){e.finish_job(d,false)})})(this);this.reset()};k.prototype.reset=function(){this.to_rip=[];this.ripped=[];this.ripped_bad=[];this.job=null};k.prototype.add_job=function(e,d){this.to_rip.push({index:e,code:d})};k.prototype.next=function(){if(!this.to_rip.length)return null;var e=this.to_rip[0];return this.job=e};k.prototype._append=function(e,d){var g=[e.length,0];g=g.concat(d);e.splice.apply(e,g)};k.prototype.finish_job=function(e,
d){for(var g=this.to_rip.length;g--;)if(this.to_rip[g].index==e){this._append(d?this.ripped:this.ripped_bad,this.to_rip.splice(g,1));return true}return false};k.prototype.has_job=function(){return this.to_rip.length>0};k.prototype.has_errors=function(){return this.ripped_bad.length>0};var q=function(){this.sPage=this.url="";this.auto_start=false;this.job_queue=new k};q.prototype.testApplication=function(){this.auto_start||this.start();this.auto_start=true};q.prototype._start=function(){var e=false;
b("input.module_code_box").each(function(){if(b(this).val()!=""){e=true;return false}});if(e){b("#ripButton").val("Waiting...").attr("disabled",true);b(".button#re-rip-errors_button").hide();b("#nextButton").hide();this.rip()}};q.prototype.rip_all=function(){this.job_queue.reset();tt.module=[];(function(e){b("input.module_code_box").each(function(d){var g=b(this).val();g!=""&&e.job_queue.add_job(d+1,g)})})(this);this._start()};q.prototype.rip_errorneous=function(){if(!this.job_queue.has_errors())throw Error("no errorneous modules to rip");
this.job_queue.to_rip=this.job_queue.ripped_bad.splice(0);this._start()};q.prototype._send_request=function(e){this.url=e;b.ajax(function(d,g){return{type:"GET",url:d.url,error:function(){NUSchedule.signals.send("on_module_rip_error",g)},success:function(s){if(s&&s.indexOf("<strong>Module Information</strong>")!=-1){d.getModule(b(s));NUSchedule.signals.send("on_module_rip_success",g)}else NUSchedule.signals.send("on_module_rip_error",g);d.ripNext()}}}(this,this.job_queue.job.index))};q.prototype.rip=
function(){var e=this.job_queue.next();if(!e||!e.code)this.ripNext();else{var d=e.code.toUpperCase(),g=b("#ay").val(),s=b("#semester").val();NUSchedule.signals.send("on_module_rip_start",e.index);this._send_request(o(g,s,d))}};q.prototype.getModule=function(e){var d=b("table.tableframe:eq(0)",e),g=b("tr:eq(1)>td:eq(1)",d).text().trim(),s=this.url;d=b("tr:eq(5)>td:eq(1)",d).text().trim().replace(/\s+(A|P)M$/,"");var v=this.ripLecture(e),A=[],x=[];e=this.ripTutorial(e);for(var y=0;y<e.length;y++){e[y].type==
"lab"&&x.push(e[y]);e[y].type=="tut"&&A.push(e[y])}e=new Module;e.code=g;e.link=s;e.exam=d;e.lecture=v;e.laboratory=x;e.tutorial=A;tt.module.push(e)};q.prototype.ripLecture=function(e){e=b("table.tableframe:eq(0) ~ table:eq(0)",e);var d=[];b("table",e).each(function(){for(var g=b("td",this).html().split("<br>"),s=g[0].trim().substring(3),v=Math.floor(g.length/2)-1,A=[],x,y,C,B,z,D,E=0;E<v;E++){x=g[E*2+1];y=g[E*2+2];C=[];B=c.exec(x);x=f(B[1]);z=parseInt(B[2]);D=parseInt(B[3]);B=B[4];if(z%100!=0)z-=
30;if(D%100!=0)D+=30;y=y.indexOf("EVEN")!=-1?2:y.indexOf("ODD")!=-1?1:0;for(var F=z;F<D;F+=100)C.push("w"+x+"t"+F);A.push(new Session(x,z,D,y,B,C))}d.push(new Part(s,"lec",A))});return d};q.prototype.ripTutorial=function(e){e=b("table.tableframe:eq(0) ~ table:eq(1)",e);var d=[];b("table",e).each(function(){for(var g=b("td",this).html().split("<br>"),s=g[0].trim().substring(3),v=s.indexOf("LABORATORY")!=-1?"lab":"tut",A=Math.floor(g.length/2)-1,x=[],y,C,B,z,D,E,F=0;F<A;F++){y=g[F*2+1];C=g[F*2+2];B=
[];z=c.exec(y);y=f(z[1]);D=parseInt(z[2]);E=parseInt(z[3]);z=z[4];C=C.indexOf("EVEN")!=-1?2:C.indexOf("ODD")!=-1?1:0;for(var G=D;G<E;G+=100)B.push("w"+y+"t"+G);x.push(new Session(y,D,E,C,z,B))}d.push(new Part(s,v,x))});return d};q.prototype.ripNext=function(){this.job_queue.has_job()?this.rip():this.rip_finish()};q.prototype.rip_finish=function(){b("#ripButton").val("Re-Scan All").attr("disabled",false);if(this.job_queue.ripped.length){this.job_queue.has_errors()&&b(".button#re-rip-errors_button").show();
tt.module.length&&b("#nextButton").show();if(this.auto_start){this.display_timetable();setTimeout("alert('Here you are. Happy testing! :)')",900)}}};q.prototype.display_timetable=function(){tt.createTable();tt.createAllNode();st.showSetFunctions();showPage3()};q.MAX_RIP_INDEX=12;return q}($),cellWidth=55,cellHeight=40;
...
TimeTable.prototype.createNode=function(b,c,f,o,k){(k=k||false)&&this.fixedArray.push(f+"_"+c.type+"_"+o);this.onTableArray.push(f+"_"+c.type+"_"+o);elemNodeMaster=document.getElementById("nodeMaster");elemSubNode=document.createElement("div");for(i=0;i<c.session.length;i++){sessionLength=c.session[i].cell.length;startingCell=c.session[i].cell[0];title=this.shortenTitle(c.title);innerHTML="<b>"+b+"</b><br/>"+title+"<br/>"+c.session[i].place;elem=document.getElementById(startingCell);t=parseInt(elem.style.top)+
1;l=parseInt(elem.style.left)+1;w=cellWidth*sessionLength-(10-sessionLength);h=cellHeight-6;nodeId=k?"f_":"n_";nodeId+=f+"_"+c.type+"_"+o+"_"+c.session[i].cell[0];elemNode=document.createElement("div");elemNode.className=k?"fixedNode":"node";elemNode.setAttribute("id",nodeId);elemNode.setAttribute("style","left:"+l+"px;top:"+t+"px;width:"+w+"px;height:"+h+"px;background-color:"+backgroundColor[f]+";color:"+fontColor[f]);elemNode.innerHTML=innerHTML;if(!k){this.signupTab(startingCell,"n_"+f+"_"+c.type+
"_"+o);tabNumber=this.getTab(startingCell,"n_"+f+"_"+c.type+"_"+o)+1;tabTop=tabNumber>3?parseInt(t)+cellHeight:t-10;tabLeft=tabNumber>3?parseInt(l)+19*(tabNumber-4):parseInt(l)+19*(tabNumber-1);tabId=k?"e_":"m_";tabId+=f+"_"+c.type+"_"+o+"_"+i;elemTab=document.createElement("div");elemTab.className=k?"fixedNodeTab":"tab";elemTab.setAttribute("id",tabId);elemTab.setAttribute("style","left:"+tabLeft+"px;top:"+tabTop+"px;width:17px;height:10px;background-color:"+backgroundColor[f]);elemSubNode.appendChild(elemTab)}subNodeId=
...
function setCookie(b,c,f,o,k,q){b=b+"="+encodeURIComponent(c);if(f)b+="; expires="+f.toGMTString();if(o)b+="; path="+o;if(k)b+="; domain="+k;if(q)b+="; secure";document.cookie=b}function getCookie(b){return RegExp("(?:; )?"+b+"=([^;]*);?").test(document.cookie)?decodeURIComponent(RegExp.$1):null}
...
var handlers=function(b){function c(d){d=d.attr("id").split("_");return b("#"+("k_"+d[1]+"_"+d[2]+"_"+d[3]))}function f(d){return c(d).siblings(".module_node_sel").attr("class","module_node")}var o=false,k,q=0,e=0;return{onmousedown:function(d){d=jQuery.browser.msie?window.event:d;var g=jQuery.browser.msie?d.srcElement:d.target;if(g.tagName=="B")g=g.parentNode;if(g.id){var s=b("#"+g.id);if(/(^m_|^n_)/.test(g.id)){k=s.parent();k.fadeTo(100,0.9);var v=k.attr("id").split("_"),A=parseInt(v[1]),x=tt.module[A];
switch(v[2]){case "lec":for(v=0;v<x.lecture.length;v++)tt.showNode(x,x.lecture[v],A,v);break;case "tut":for(v=0;v<x.tutorial.length;v++)tt.showNode(x.code,x.tutorial[v],A,v);break;case "lab":for(v=0;v<x.laboratory.length;v++)tt.showNode(x.code,x.laboratory[v],A,v);break}o=true;q=d.screenX;e=d.screenY}if(/^k_/.test(g.id))if(g.className=="module_node"){g.className="module_node_sel";tt.swapNode(s,f(s))}else if(g.className=="module_node_sel"){g.className="module_node_1";tt.swapNode(s,s,true)}else{g.className=
"module_node";tt.removeNode(s)}switch(g.tagName){case "INPUT":case "A":case "TEXTAREA":case "SELECT":case "OPTION":break;default:return false}}},onmouseup:function(d){d=jQuery.browser.msie?d.srcElement:d.target;if(d.tagName=="B")d=d.parentNode;if(d.id){var g=b("#"+d.id);if(o){for(var s=document.getElementById("tempNodeMaster");s.hasChildNodes();)s.removeChild(s.lastChild);tt.resetTempCell();k.css("left","0px").css("top","0px").fadeTo(100,0.6)}o=false;if(/(^b_|^t_)/.test(d.id)){tt.swapNode(g,k);f(g);
c(g).attr("class","module_node_sel")}}},onmousemove:function(d){if(jQuery.browser.msie)d=window.event;if(o){k.css("left",d.screenX-q+"px");k.css("top",d.screenY-e+"px")}},onmouseover:function(d){d=jQuery.browser.msie?d.srcElement:d.target;if(d.tagName=="B")d=d.parentNode;/(^b_|^t_)/.test(d.id)&&b("#"+d.id).siblings().andSelf().css("backgroundColor","#a55").css("zIndex",30)},onmouseout:function(d){d=jQuery.browser.msie?d.srcElement:d.target;if(d.tagName=="B")d=d.parentNode;/(^b_|^t_)/.test(d.id)&&
