...
"";this.auto_start=false};s.prototype.testApplication=function(){this.auto_start||this.start();this.auto_start=true};s.prototype.start=function(){var k=false;f(function(d,g){if(g.val()!="")return k=true});if(k){b("#ripButton").val("Waiting...").mouseup(function(){return false});b("#nextButton").hide();f(function(d,g){g.val()!=""&&NUSchedule.signals.send("on_module_rip_start",d)});this.rip_index=1;tt.module=[];this.rip()}};s.prototype._send_request=function(k){this.url=k;var d=this;b.get(this.url,
function(g){if(g.indexOf("<strong>Module Information</strong>")!=-1){d.sPage=g;d.$page=b(g);d.getModule();NUSchedule.signals.send("on_module_rip_success",d.rip_index)}else NUSchedule.signals.send("on_module_rip_error",d.rip_index);d.ripNext()})};s.prototype.rip=function(){var k=b("#code"+this.rip_index).val().toUpperCase(),d=b("#ay").val(),g=b("#semester").val();if(k!="")this._send_request(o(d,g,k));else{NUSchedule.signals.send("on_module_rip_blank",this.rip_index);this.ripNext()}};s.prototype.getModule=
function(){var k=b("table.tableframe:eq(0)",this.$page),d=b("tr:eq(1)>td:eq(1)",k).text().trim(),g=this.url;k=b("tr:eq(5)>td:eq(1)",k).text().trim().replace(/\s+(A|P)M$/,"");for(var x=this.ripLecture(),v=[],A=[],q=this.ripTutorial(),y=0;y<q.length;y++){q[y].type=="lab"&&A.push(q[y]);q[y].type=="tut"&&v.push(q[y])}q=new Module;q.code=d;q.link=g;q.exam=k;q.lecture=x;q.laboratory=A;q.tutorial=v;tt.module.push(q)};s.prototype.ripLecture=function(){var k=b("table.tableframe:eq(0) ~ table:eq(0)",this.$page),
d=[];b("table",k).each(function(){for(var g=b("td",this).html().split("<br>"),x=g[0].trim().substring(3),v=Math.floor(g.length/2)-1,A=[],q,y,C,B,z,D,E=0;E<v;E++){q=g[E*2+1];y=g[E*2+2];C=[];B=c.exec(q);q=e(B[1]);z=parseInt(B[2]);D=parseInt(B[3]);B=B[4];if(z%100!=0)z-=30;if(D%100!=0)D+=30;y=y.indexOf("EVEN")!=-1?2:y.indexOf("ODD")!=-1?1:0;for(var F=z;F<D;F+=100)C.push("w"+q+"t"+F);A.push(new Session(q,z,D,y,B,C))}d.push(new Part(x,"lec",A))});return d};s.prototype.ripTutorial=function(){var k=b("table.tableframe:eq(0) ~ table:eq(1)",
this.$page),d=[];b("table",k).each(function(){for(var g=b("td",this).html().split("<br>"),x=g[0].trim().substring(3),v=x.indexOf("LABORATORY")!=-1?"lab":"tut",A=Math.floor(g.length/2)-1,q=[],y,C,B,z,D,E,F=0;F<A;F++){y=g[F*2+1];C=g[F*2+2];B=[];z=c.exec(y);y=e(z[1]);D=parseInt(z[2]);E=parseInt(z[3]);z=z[4];C=C.indexOf("EVEN")!=-1?2:C.indexOf("ODD")!=-1?1:0;for(var G=D;G<E;G+=100)B.push("w"+y+"t"+G);q.push(new Session(y,D,E,C,z,B))}d.push(new Part(x,v,q))});return d};s.prototype.ripNext=function(){if(++this.rip_index<=
12)this.rip();else{b("#ripButton").val("Re-Scan All").mouseup(this.start);tt.module.length>0&&b("#nextButton").show();if(this.auto_start){tt.createTable();tt.createAllNode();st.showSetFunctions();showPage3();setTimeout("alert('Here you are. Happy testing! :)')",900)}}};s.MAX_RIP_INDEX=12;return s}($),cellWidth=55,cellHeight=40;
